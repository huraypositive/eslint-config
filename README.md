...
push 후 GitHub Actions가 자동으로 **"[배포] 패키지 버전 업데이트"** PR을 생성합니다.
...
#### Step 4. PR 머지 → npm 자동 배포

PR을 검토하고 머지하면 GitHub Actions가 자동으로 npm에 배포합니다.
...
pnpm changeset   # 어떤 패키지를, 어떤 버전으로, 왜 올리는지 기록
  ↓
git push         # GitHub Actions가 버전 업데이트 PR 자동 생성
  ↓
PR 머지          # GitHub Actions가 npm 자동 배포 + 태그 생성
```
